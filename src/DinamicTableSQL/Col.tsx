...
import { Animated, FlatList, PanResponder, Text, TextInput, TextStyle, TouchableOpacity, View } from "react-native";
import { CellStyle, ColData, Colors, DinamicTableSQL } from ".";
...
export type ColPropsType<T> = {
    ...
    dataType: DataType,
    dateFormat?: string,
    // renderHeader?: (props: { textStyle: TextStyle, dinamicTable: DinamicTableSQL<T> }) => any,
    width: number,
    // data: (props: { row: T, index: number }) => any,
    format?: (props: { data: any, row: T, index: number, textStyle: TextStyle }) => any,
    customComponent?: (props: { data: any, dataFormat: any, row: T, index: number, textStyle: TextStyle, dinamicTable: DinamicTableSQL<T>, colData: ColData }) => any,
    ...
    excelFormat?: string,
    sumExcel?: boolean,
    usePermission?: (props: { data: any, row: T, index: number, textStyle: TextStyle, dinamicTable: DinamicTableSQL<T>, colData: ColData }) => boolean,
    renderFooter?: (props: { textStyle: TextStyle, dinamicTable: DinamicTableSQL<T> }) => any,
    loadListOptions?: (props: { dinamicTable: DinamicTableSQL<T>, colData: ColData }) => Promise<any[]>,
}
...
export default class Col<T> extends React.Component<ColPropsType<T>> {
    ...
    render() {
        ...
        return <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            >
                <TouchableOpacity onPress={this.showPopup.bind(this)} style={{ width: "100%", height: "100%", justifyContent: "center", alignItems: "flex-end", flexDirection: "row", paddingStart: 2, paddingBottom: 2, }}>
                    {this.props.labelIcon}
                    <Text style={[{ color: colors.text, overflow: "hidden", flexWrap: "wrap", textAlign: "center" }, this.props?.dinamicTableInstance?.props?.textStyle, this.props.textStyle]}
